...
def initial():
    ...
    goal_list = []
    first_node_id = 1
    start_cords_x = int(input("please enter the starting x coordinate between 1 and 400: \n"))
    start_cords_y = int(input("please enter the starting y coordinate between 1 and 250: \n"))
...
while not done:
    ...
    for i in range(len(x_a)-1):
            ...
            # y = (All_list[ind-1].node_loc[0])
            # x = (All_list[ind-1].node_loc[1])
            # id = All_list[ind-1].parent_id
            # y_parent = (All_list[id-1].node_loc[0])
            # x_parent = (All_list[id-1].node_loc[0])
            # while(id>0):
            #     x.append(All_list[id-1].node_loc[0])
            #     y.append(All_list[id-1].node_loc[1])
            # id = All_list[id - 1].parent_id
            pygame.draw.line(gameDisplay, y, (n*x_a[i],n*(display_height-y_a[i])),(n*x_a[i+1],n*(display_height-y_a[i+1])), 3)
            pygame.display.flip()
            pygame.time.wait(4)
    done = True
